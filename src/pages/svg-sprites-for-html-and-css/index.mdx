...
import symbols from "./media/sprite-symbols.svg";
import hybrid from "./media/sprite-hybrid.svg";
...
<button type="button" class="icon-button">
  <svg>
    <use xlink:href={`${symbols}#search`} />
  </svg>
  <span>Search</span>
</button>
<button type="button" class="icon-button">
  <svg>
    <use xlink:href={`${symbols}#close`} />
  </svg>
  <span>Close</span>
</button>
...
<style
  set:html={`
.var-background::before {
	background-image: url(${symbols}#search);
	content: "";
	width: 2rem;
	aspect-ratio: 1;
}`}
...
<style
  set:html={`
.var-view::before {
	background-image: url(${hybrid}#search-view);
}
`}
/>
...
.var-mask::before {
	background: currentColor;
	mask-image: url(${hybrid}#search-view);
}
`}
/>
